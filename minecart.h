...
constexpr rectangle2 minecartEditedTextureRect = crectangle2(0, 0, minecartBottomTextureRect.size.x(), minecartBottomTextureRect.size.y() + minecartTopTextureRect.size.y());
constexpr int minecartWheelPixelSize = 6;

constexpr vec2 minecartHitboxSize = cvec2(1, 0.7);

constexpr fp minecartVolume = minecartHitboxSize.volume() * minecartHitboxSize.x();
constexpr fp minecartWeight = minecartVolume * ironWeightPerCubicMeter;

constexpr fp minecartPixelSize = minecartHitboxSize.x() / minecartEditedTextureRect.size.x();

constexpr vec2 minecartRenderSize = cvec2(minecartEditedTextureRect.size) * minecartPixelSize;
constexpr vec2 minecartRotationCentre = cvec2(minecartRenderSize.x() * 0.5, 0);
