...
struct block :IDestructable
{
	...
	bool correctTool(const itemID& tool);
	...
	void render(const brush0Type& b, crectangle2& brushRect, crectangle2& blockRect, const renderData& targetData, blockData* const data, blockContainer* containerIn, cbool& renderAnimation = false, const std::optional<vec2>& rotationCentreWorld = std::nullopt) const;
