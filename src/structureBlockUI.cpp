...
void structureBlockUI::mouseDown(cveci2 &position, cmb &button) {
    ...
    if (highestChild == modeButton) {
        temporaryStructureBlockData->mode = (structureBlockMode) (
                (int) temporaryStructureBlockData->mode + 1);
        ...
        *((structureBlockData *) currentHuman->selectedContainerContainer->getBlockData(
                currentHuman->selectedContainerPosition)) = *temporaryStructureBlockData;
