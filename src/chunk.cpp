...
chunk::chunk(dimension *dimensionIn, cveci2 &chunkCoordinates) : dimensionIn(dimensionIn),
                                                                 chunkCoordinates(
                                                                         chunkCoordinates) {
...
void chunk::spawnMobs() {
    cint spawnTries =
            spawnCooldown <= spawnTreshold ? spawnRate : spawnCooldown <= doubleSpawnRateTreshold
    ...
    for (int i = 0; i < spawnTries; i++) {
        ...
        for (const entityID &id: mobList) {
            spawnWeights[(int) id] = ((mobData *) entityDataList[(int) id])->getSpawnWeight(
                    dimensionIn, blockSpawningIn);
