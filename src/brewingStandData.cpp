			...
			if (ticksBrewed >= brewingBatchTime)
			{
				...
				for (int i = 0; i < brewingStandPotionCapacity; i++)
				{
					if (potionSlots[i]->slots[0].count)
					{
						...
						potionData* currentData = canAddEffects(currentItem) ? (potionData*)potionSlots[i]->slots[0].data : nullptr;
						if ((ingredientItem == itemID::gunpowder) && (currentItem == itemID::potion))
						{
							potionSlots[i]->slots[0].stackItemID = itemID::splash_potion;
						}
						else if ((ingredientItem == itemID::dragon_breath) && (currentItem == itemID::splash_potion))
						{
							potionSlots[i]->slots[0].stackItemID = itemID::lingering_potion;
						}
						else if (((ingredientItem == itemID::redstone) && currentData) && currentData->effectsToAdd.size())
						{
							...
							for (statusEffect& effectToMakeLonger : currentData->effectsToAdd)
							{
								if (effectToMakeLonger.potency < 2)
								{
									...
									case statusEffectID::speed:
										effectToMakeLonger.ticksDuration = 8 * 60 * ticksPerRealLifeSecond;
										break;
									case statusEffectID::slowness:
										effectToMakeLonger.ticksDuration = 4 * 60 * ticksPerRealLifeSecond;
										break;
									...
									case statusEffectID::poison:
										effectToMakeLonger.ticksDuration = 90 * ticksPerRealLifeSecond;
										break;
						...
						else
						{
							...
							const bool isWaterBottle = (currentItem == itemID::potion) && (currentData->effectsToAdd.size() == 0);
							if (isWaterBottle)
							{
								...
								{
								case (itemID)blockID::nether_wart:
								{
								...
								{
									((potionData*)(modifiedPotionStack.data))->effectsToAdd.push_back(statusEffect(statusEffectID::weakness, 90 * ticksPerRealLifeSecond));
								}
							...
							{
								if ((currentItem == itemID::awkward_potion) || (canAddEffects(currentItem) && (currentItem != itemID::potion) && (currentData->effectsToAdd.size() == 0)))
								{
									if (currentItem == itemID::awkward_potion)
									{
										itemStack modifiedPotionStack;
										modifiedPotionStack = itemStack(itemID::potion, 1, new potionData());
										potionSlots[i]->slots[0].add(-1);
									...
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::strength, 5 * 60 * ticksPerRealLifeSecond));
									}
									break;
									case itemID::ghast_tear:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::regeneration, 45 * ticksPerRealLifeSecond));
									}
									break;
									case itemID::golden_carrot:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::nightVision, 5 * 60 * ticksPerRealLifeSecond));
									}
									break;
									case itemID::magma_cream:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::fireResistance, 3 * 60 * ticksPerRealLifeSecond));
									}
									break;
									case itemID::glistering_melon_slice:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::instantHealth));
									}
									break;
									case itemID::phantom_membrane:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::slowFalling, 90 * ticksPerRealLifeSecond));
									}
									break;
									case itemID::rabbit_foot:
									{
										currentData->effectsToAdd.push_back(statusEffect(statusEffectID::jumpBoost, 3 * ticksPerRealLifeMinute));
									}
									break;
									}
								}
