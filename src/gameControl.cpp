...
void gameControl::render(cveci2 &position, const texture &renderTarget) {
    ...
    if (settings::renderDebugData) {
        ...
        if (player->entityType == entityID::human) {
            ...
            text += std::wstring(L"selected light level:\tinternal sunlight:") + std::to_wstring(
                    currentHuman->selectedBlockContainer->getInternalSunLightLevel(
                            currentHuman->selectedBlockPosition)) +
                    std::wstring(L"\tblock light:") + std::to_wstring(
                    currentHuman->selectedBlockContainer->getBlockLightLevel(
                            currentHuman->selectedBlockPosition)) +
                    std::wstring(L"\tpower level:") + std::to_wstring(
                    currentHuman->selectedBlockContainer->getPowerLevel(
                            currentHuman->selectedBlockPosition)) + std::wstring(L"\n");
        }

        if (player->dimensionIn->identifier == dimensionID::overworld) {
            overWorld *currentOverWorld = (overWorld *) player->dimensionIn;
            text += std::wstring(L"noise values:\televation:") + std::to_wstring(
                    currentOverWorld->biomeElevationNoise->evaluate(vec1(player->position.x))) +
                    std::wstring(L"\ttemperature:") + std::to_wstring(
                    currentOverWorld->biomeTemperatureNoise->evaluate(vec1(player->position.x))) +
                    std::wstring(L"\thumidity:\t") + std::to_wstring(
                    currentOverWorld->biomeHumidityNoise->evaluate(vec1(player->position.x))) +
                    std::wstring(L"\tcloudiness:\t") + std::to_wstring(
                    currentOverWorld->cloudThicknessNoise->evaluate(
                            vec1(currentWorld->currentTime))) + std::wstring(L"\n");
        }
        ...
        text += std::wstring(L"biome:\t") + getClassName(
                *currentWorld->biomeList[(int) player->dimensionIn->getBiome(player->position)]) +
                std::wstring(L"\n");
        ...
        text += std::wstring(L"sound count:\t") + std::to_wstring(playingSoundCount) +
                std::wstring(L"\n");
        text += std::wstring(L"fps:\t") + std::to_wstring(
                1.0 / microsectosec(currentBenchmark->measureTotalBenchmarkTime())) +
                std::wstring(L"\nmicroseconds used:\ntotal: ") +
    ...
    */
    ...
    if (focusedChild) {
        fillTransparentRectangle(crosshairTextureRect,
                                 crectangle2(cvec2(mousePositionPixels), cvec2()).expanded(
                                         (fp) crosshairTextureRect.size.x *
                                         settings::videoSettings::guiScale), *iconsTexture,
                                 renderTarget);
    }
}
...
void gameControl::processInput() {
    ...
    if (focusedChild) {
        ...
        veci2 pos;
        if (player->hotbarSlots->getSlot(mousePositionPixels, cveci2(hotbarDrawRect.pos0 +
                                                                     (hotbarSpacing -
                                                                      hotbarItemDisplaySize) / 2 *
                                                                     hudScale),
...
void gameControl::renderGame(crectanglei2 &rect, const texture &renderTarget, cbool &renderHUD) {
                ...
                if (currentWorld->xray) {
                    ...
                    if (enclosed) {
                        ...
                        colorf *cornerColorPtr = interpolators.getValueReferenceUnsafe(
                                cveci2(relativePosition)).cornerColors;
                        // for each corner
                            ...
                            for (int cornerX = relativePosition.x;
                                ...
                                for (int cornerCornerY = cornerY;
                                    ...
                                    for (int cornerCornerX = cornerX; cornerCornerX < cornerX +
                                                                                      averageDistance2; cornerCornerX++) {
                                        for (size_t lightDataTypeIndex = 0; lightDataTypeIndex <
                                                                            (int) lightLevelID::count; lightDataTypeIndex++) {
        ...
        if (collides2d(worldRect, e->calculateHitBox())) {
            ...
            gameRenderData entityRenderData = gameRenderData(
                    mat3x3::cross(targetData.worldToRenderTargetTransform,
                                  mat3x3::translate(movement)), targetData.renderTarget,
    ...
    if (player->entityType == entityID::human) {
        ...
        if (blockList[(int) currentHuman->selectedBlockContainer->getBlockID(
                selectionPosToDraw)]->canReplaceBlock) {
        ...
        blockScreenRect = mat3x3::cross(targetData.worldToRenderTargetTransform,
                                        currentHuman->selectedBlockContainer->containerToRootTransform).multRectMatrix(
                crectangle2(cvec2(selectionPosToDraw) +
                            (currentHuman->selectedBlockContainer->speedInRoot *
                             targetData.secondsOffset), cvec2(1)));
        ...
        if (currentHuman->selectedBlockDamage) {
            ...
            cfp &destroyStage = currentHuman->selectedBlockDamage /
                                (blockList[(int) currentHuman->selectedBlockContainer->getBlockID(
                                        currentHuman->selectedBlockPosition)]->hardness *
                                 10); // 0 to 1
        ...
        if (isMob(player->entityType)) {
            ...
            crectangle2 &unreachableCrosshairDrawRect = crectangle2(
                    targetData.worldToRenderTargetTransform.multPointMatrix(
                            currentMousePositionWorld), cvec2()).expanded(
            ...
            if (isHumanoid(player->entityType)) {
                ...
                if (player->entityType == entityID::human) {
                    ...
                    if (currentHuman->experience) {
                        ...
                        crectangle2 &expBarforeGroundTextureRect = crectangle2(0, 162, 182 *
                                                                                       progressToNextLevel,
                                                                               5);
                        ...
                        if (currentLevel) {
                            ...
                            f.DrawString(str, crectangle2((rect.w - size.x) * 0.5,
                                                          currentYrowOffset + expBarSize.y -
                                                          scaledBarOffset, size.x, size.y),
                                         targetData.renderTarget);
    ...
    for (entity *e: nearEntities) {
        if (isBossMob(e->entityType)) {
            crectangle2 bossBarBackGroundTextureRect = crectangle2(0, 0x100 -
                                                                      (((bossBarIndex * 2) + 1) *
                                                                       barTextureSize.y),
                                                                   barTextureSize.x,
                                                                   barTextureSize.y);
            ...
            if (e->health > 0) {
                ...
                crectangle2 bossBarForeGroundTextureRect = crectangle2(0, 0x100 -
                                                                          (((bossBarIndex * 2) +
                                                                            2) * barTextureSize.y),
...
void gameControl::addTouchInput() {
    ...
    addEventHandlers(&gameControl::onJoystickTouch, interactJoystick->onMouseDown);
    addEventHandlers(&gameControl::onJoystickTouchEnd, interactJoystick->onMouseUp);
    addEventHandlers(&gameControl::onBackgroundTouch, onMouseDown);
}
