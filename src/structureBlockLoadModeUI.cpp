...
void structureBlockLoadModeUI::mouseDown(cveci2& position, cvk& button)
{
	...
	{
		structure* s = getStructureByName(structureFolder + ((gameControl*)parent)->structureBlockOptions->temporaryStructureBlockData->structureName);
		if (s)
		{
...
void structureBlockLoadModeUI::render(cveci2& position, const texture& renderTarget)
{
	...
	structure* s = getStructureByName(structureFolder + structureBlockOptions->temporaryStructureBlockData->structureName);
	if (s)
	{
