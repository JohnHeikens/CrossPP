...
bool structure::serialize(const std::wstring& path, cbool& write)
{
	...
	if (s.serializeArray(std::wstring(L"blocks"), (int*&)blockIDArray.baseArray, (int)blockIDArray.size.volume()) && s.converter)
	{
		idConverter::convertArray((itemID*&)blockIDArray.baseArray, (int)blockIDArray.size.volume(), s.converter->itemIDConverter);
	}
	if (!write)
	{
...
void structure::createStructure(dimension& dimensionIn, crectanglei2& rect, cbool& includeEntities)
{
	...
	for (veci2 pos = veci2(); pos.y <= rect.size.y; pos.y++)
	{
		for (pos.x = 0; pos.x <= rect.size.x; pos.x++)
		{
			...
			if (cloneTo)
			{
				blockData* const& cloneFrom = dimensionIn.getBlockData(pos + rect.pos0, chunkLoadLevel::updateLoaded);
				if (cloneFrom)
				{
	...
	if (includeEntities)
	{
		...
		for (entity* e : entitiesInBox)
		{
			if (e->entityType != entityID::particle && e->entityType != entityID::item)
			{
				...
				entity* clonedEntity = createEntity(e->entityType, nullptr, e->position - rect.pos0);
...
{
	return getAffectedPositions(crectanglei2(blockIDArray.getClientRect()), [this, id](cveci2& pos) {
...
void* structure::getArrayValuePointerUnsafe(cveci2& position, const arrayDataType& dataType, const chunkLoadLevel& minimalLoadLevel)
{
	...
	for (size_t i = 0; i < structureList.size(); i++)
	{
		...
		if (currentFolder.compare(seekFolder) == 0)
		{
			...
			else
			{
				...
				{
					int fileNumber;
					if ((!convertToInt(std::wstring(1, currentFileName[name.length() - 1]), fileNumber)) && convertToInt(currentFileName.substr(name.length()), fileNumber))
					{
						foundStructures.push_back(s);
					}
				}
