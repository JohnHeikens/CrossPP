...
struct structure :IDestructable, blockContainer
{
	...
	stdFileSystem::path pathWithoutExtension = stdFileSystem::path();
	...
	inline structure(const stdFileSystem::path pathWithoutExtension) : structure() { this->pathWithoutExtension = pathWithoutExtension; }
	bool serialize(const std::wstring& path, cbool& write);
